...
function config() {
  ...
  return {
    ...
    secret,
    bucket: process.env.SUPABASE_PHOTO_BUCKET ?? DEFAULT_BUCKET,
  };
}
...
export async function uploadPrivatePhoto(key: string, bytes: ArrayBuffer, contentType: string) {
  ...
  const response = await fetch(
    `${baseURL}/storage/v1/object/${encodeURIComponent(bucket)}/${encodeURIComponent(key)}`,
    {
      ...
      headers: { ...headers(contentType), 'x-upsert': 'false', 'cache-control': 'no-store' },
      body: bytes,
    },
  );
...
export async function deletePrivatePhoto(key: string) {
  ...
  const response = await fetch(
    `${baseURL}/storage/v1/object/${encodeURIComponent(bucket)}/${encodeURIComponent(key)}`,
    { signal: AbortSignal.timeout(15000), method: 'DELETE', headers: headers() },
  );
...
export async function createPrivatePhotoURL(key: string): Promise<string> {
  const { baseURL, bucket } = config();
  const response = await fetch(
    `${baseURL}/storage/v1/object/sign/${encodeURIComponent(bucket)}/${encodeURIComponent(key)}`,
    {
      method: 'POST',
      signal: AbortSignal.timeout(45000),
  ...
  return body.signedURL.startsWith('http')
    ? body.signedURL
    : `${baseURL}/storage/v1${body.signedURL}`;
}
