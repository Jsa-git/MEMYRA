...
import { createPrivatePhotoURL } from '../../../../../src/server/photo-storage';
...
export async function POST(_request: Request, { params }: { params: Promise<{ id: string }> }): Promise<Response> {
  try {
    ...
    const photo = await getDatabase().photoRecord.findFirst({ where: { id: id.data, journey: { userId: actor.userId } }, select: { storageKey: true } });
    if (!photo) return NextResponse.json({ code: 'NOT_FOUND' }, { status: 404 });
    return NextResponse.json({ url: await createPrivatePhotoURL(photo.storageKey), expiresIn: 60 }, { headers: { 'cache-control': 'private, no-store' } });
  } catch (error) {
